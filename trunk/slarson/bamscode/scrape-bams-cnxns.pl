...
#this program reads an xml file with information about bams connections
# and looks up URLs associated with each connection
# It takes the information it finds and updates the xml file.

my $reader = new XML::LibXML::Reader(location => "cnxns.xml")
...
while ($reader->read) {
	...
	# for nodes that talk about connections
	if ($reader->name eq "Connection") {
	    my $url = $reader->getAttribute("url");
	    $url = "http://brancusi.usc.edu/bkms/brain/conef-det.php?".$url;
	    print $url;
	    $agent->get($url);
	    ...
	    my $projDetailsRef = scrapeProjDetails($stream);
	    my $refURLRef = scrapeRefUrl($stream);
	    
	    print @$projDetailsRef;
	    last;
	}
    }
}
