...
my %idsNames = ();
my $reader = new XML::LibXML::Reader(location => "molecules.owl");
...
while ($reader->read) {
	...
	if ($reader->name eq "bams_molecule_id") {
	    $reader->read;
...
my $doc = $parser->parse_file("brainparts-canonical-with-cells.xml");

my $xc = XML::LibXML::XPathContext->new($doc);

my @nodes = $xc->findnodes('/Parts/Part/Molecules/Molecule');
